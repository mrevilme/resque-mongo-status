...
resque-mongo-status is an extension to the resque queue system that provides simple trackable jobs.
...
resque-mongo-status *requires >= 1.0.0* (though I recommend getting the latest stable version). 
You can download MongoDB here: http://www.mongodb.org/ or install it 
using homebrew (brew install mongodb).s

Install the resque-status gem (which will pull in the dependencies).

  gem install resque-mongo-status
...
With newer Rails add this to your Gemfile:
  ...
  # Gemfile
  gem 'resque-mongo-status', :require 'resque-status'
...
You might not want to keep the statuses around forever  By setting expire_in, all statuses
...
Source: http://github.com/mrevilme/resque-mongo-status
Examples: http://github.com/mongo/resque-mongo-status/tree/master/examples
Resque-Mongo: https://github.com/streamio/mongo-resque
            
== Thanks

Resque is awesome, @defunkt needs a shout-out. Also @streamio for making the mongo fork
...
Copyright (c) 2012 Emil Palm. See LICENSE for details.
